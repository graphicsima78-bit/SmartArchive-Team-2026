...
class MainWindow(QMainWindow):
    ...
    CATEGORY_TABS = {
        ...
        "media": ("صوت و رسانه", ["audio", "video"], "بایگانی تخصصی موسیقی، آلبوم‌ها، پادکست و فیلم‌ها."),
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("ArchivePro Studio v43.0 [Media Restore]")
    ...
    def _build_ui(self):
        ...
        self.main_layout.addWidget(self.tabs, 1) # This '1' makes it expand and fill space
        ...
        # FIXED BOTTOM SECTION (Will not collapse)
        bottom_container = QGroupBox("وضعیت عملیات")
        bottom_layout = QVBoxLayout(bottom_container)
        
        controls = QHBoxLayout()
        self.stop_btn = QPushButton("توقف عملیات")
        ...
        # Permanent Green Progress Bar
        self.global_progress = QProgressBar()
        self.global_progress.setRange(0, 100)
        self.global_progress.setValue(0)
        self.global_progress.setTextVisible(True)
        self.global_progress.setFormat("پیشرفت: %p%")
        self.global_progress.setMinimumHeight(28)
    ...
    def _build_category_tab(self, key, title, focus_types, description):
        ...
        path_group = QGroupBox(f"تنظیمات درایو و پوشه ({title})")
        ...
        src_btn.clicked.connect(lambda: self._browse_folder(src, "انتخاب پوشه ورودی"))
        dst_btn.clicked.connect(lambda: self._browse_folder(dst, "انتخاب پوشه خروجی"))
        ...
        opts = {"source": src, "destination": dst, "focus": focus_types, "delete": QCheckBox("حذف فایل‌های اصلی پس از جابه‌جایی")}
        sec = QGroupBox("امنیت فایل")
        sl = QVBoxLayout(sec)
        sl.addWidget(opts["delete"])
        layout.addWidget(sec)

        start = QPushButton(f"شروع بایگانی {title}")
    ...
    def _build_report_tab(self):
        ...
        self.tabs.addTab(tab, "گزارش لحظه‌ای")
    ...
    def _on_finished(self):
        self._append_log("==================================")
        self._append_log("عملیات با موفقیت به پایان رسید.")
        self._append_log("==================================")
        self.global_progress.setValue(0)
        self.stop_btn.setEnabled(False)
    ...
    def stop_processing(self):
        if self.worker:
            self.worker.stop()
            self._append_log("توقف عملیات توسط کاربر...")

    def start_processing(self, key):
        c = self.tab_config[key]
        s, d = c["source"].text().strip(), c["destination"].text().strip()
        if not s or not d:
            QMessageBox.warning(self, "خطا", "لطفاً مسیرهای مبدأ و مقصد را مشخص کنید.")
            return
