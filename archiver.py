...
# --- INTERNAL EXTENSION LISTS (Self-Contained) ---
...
class FileAnalyzer:
    ...
    def basic_metadata(file_path):
        ...
        return {"size": stat.st_size, "mtime": stat.st_mtime, "mime": mime_type or "application/octet-stream"}
...
class ArchiveWorker(QObject):
    ...
    def __init__(self, source_dir, dest_dir, **kwargs):
        ...
        self.use_persian = bool(kwargs.get('use_persian', False))
        self.move_mode = bool(kwargs.get('delete_after_copy', False))
        self.reprocess_archived = bool(kwargs.get('reprocess_archived', False))
        self.content_analysis = bool(kwargs.get('content_analysis', False))
        self.quarantine_duplicates = bool(kwargs.get('quarantine_duplicates', False))
        self.focus_types = set(kwargs.get('focus_types') or [])
        self.quick_transfer = bool(kwargs.get('quick_transfer', False))
        self.project_config = dict(kwargs.get('project_config') or {})
        self.family_location_when_dated = bool(kwargs.get('family_location_when_dated', True) and self.use_date)
        ...
        self.fast_image = FastImageAnalyzer(use_ocr=kwargs.get('use_ocr', True))
        self.taxonomy = TaxonomyManager()

    def run(self):
        self.log.emit("Starting Archive Engine v26.0...")
