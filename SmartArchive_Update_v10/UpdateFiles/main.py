...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.gemma_checkbox = QCheckBox("تحلیل محتوا برای دسته‌بندی‌نشده‌ها (کندتر)")
        self.gemma_checkbox.setChecked(False)
        self.gemma_checkbox.setToolTip("ابتدا دسته‌بندی سریع اجرا می‌شود؛ فقط تصاویر نامشخص با Gemma تحلیل می‌شوند.")
        self.quarantine_duplicates_checkbox = QCheckBox("انتقال تکراری‌های دقیق به پوشه تکراری‌ها")
        self.quarantine_duplicates_checkbox.setChecked(False)
        self.quarantine_duplicates_checkbox.setToolTip("فقط فایل‌های با SHA-256 یکسان را به مقصد\\تکراری‌ها منتقل می‌کند؛ حذف دائمی انجام نمی‌شود.")
        ...
        safety_options.addWidget(self.quarantine_duplicates_checkbox)
