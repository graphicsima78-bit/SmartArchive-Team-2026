...
class ArchiveWorker(QObject):
    ...
    def __init__(
        ...
        self.fast_image = FastImageAnalyzer()
    ...
    def _image_hint_to_folder(self, ai_hint):
        ...
        if category in {"documents", "document"}:
            return ["تصاویر", "اسناد", subcategory]
    ...
    def _archive_preview(self, image_path, main_parts, main_file):
        ...
        if main_file.suffix.lower() in VECTOR_EXTS:
            # A visual companion for EPS/AI/PSD/etc. stays directly beside its graphic file.
            destination = self._dest_path(image_path, main_parts)
            file_type = "paired_graphic_image"
            log_label = "Archived paired graphic image"
        else:
            destination = self._preview_path(main_parts, image_path)
            file_type = "preview"
            log_label = "Archived preview"

        self._copy_or_move(image_path, destination)
        self.db.insert_file(str(image_path), str(destination), md5, sha256, metadata["size"], metadata["mtime"], metadata["mime"], {"type": file_type, "paired_with": str(main_file), **metadata})
        self.log.emit(f"{log_label}: {image_path.name} -> {destination}")
    ...
    def _archive_image(self, image_path):
        ...
        # Phase 1: quick filename and lightweight offline analysis.
        ai_hint = self.gemma.quick_classify_image(image_path)
        if float(ai_hint.get("confidence", 0) or 0) < 0.5:
            fast_hint = self.fast_image.analyze(image_path)
            if float(fast_hint.get("confidence", 0) or 0) >= 0.5:
                ai_hint = fast_hint
            # Phase 2: expensive Gemma analysis only for files still unclassified.
            elif self.use_gemma and self.gemma_available:
                ...
                ai_hint = {
                    ...
                    "subcategory": "other",
                    "confidence": 0.0,
                    "reason": "Fast mode: moved to unclassified",
                }
    ...
    def run(self):
        try:
            ...
            if self.use_gemma:
                self.log.emit("Checking Gemma Vision service for unclassified images...")
                self.gemma_available = self.gemma.is_available()
                if self.gemma_available:
                    self.log.emit("Content analysis is enabled. Gemma will run only for images still unclassified after fast analysis.")
                else:
                    self.log.emit("Gemma Vision is unavailable. Remaining images will go to تصاویر\\دسته‌بندی نشده without waiting.")
            else:
                self.log.emit("Fast classification is enabled. Remaining images will go to تصاویر\\دسته‌بندی نشده.")
