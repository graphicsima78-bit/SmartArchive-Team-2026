...
title Apply SmartArchive Update v7
set "PROJECT=C:\Users\amin\Desktop\SmartArchive"
set "UPDATE=%~dp0UpdateFiles"
set "BACKUP=%USERPROFILE%\Desktop\SmartArchive_Backup_v7"
...
for %%F in (main.py archiver.py file_analyzer.py gemma_connector.py audio_analyzer.py requirements.txt) do (
    if exist "%PROJECT%\%%F" copy /Y "%PROJECT%\%%F" "%BACKUP%\%%F" >nul
    copy /Y "%UPDATE%\%%F" "%PROJECT%\%%F" >nul
)

echo.
echo SmartArchive Update v7 was applied successfully.
